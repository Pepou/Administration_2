...
#        nb_page_total = 1
        ...
        #Titre
        c.setFont("Arial", 16)
        c.drawString(6.8*cm,20.3*cm,"RAPPORT DE CARTOGRAPHIE")
        ...
        #n° Cartographie
        c.setFont("Arial", 9)
        c.drawString(1.7*cm,28.4*cm,"RAPPORT DE CARTOGRAPHIE N° {}".format(administratif["num_rapport"]))
        c.setFont("Arial", 8)
        ...
        c.drawString(9.8*cm,27*cm,"Matériel")
        c.rect(1.3*cm, 26.9*cm, 18.1*cm, 0.4*cm , stroke=1, fill=0)
        
        
        c.setFont("Arial", 10)        
        c.drawString(1.7*cm,26.2*cm,"Site de : {}".format(administratif["ville_client"]))
        
        c.drawString(1.7*cm,25.2*cm,"Model : {}".format(administratif["model_enceinte"]))
        
        c.drawString(1.7*cm,24.2*cm,"Identification : {}".format(administratif["ident_enceinte"]))
        ...
        c.drawString(13*cm,26.2*cm,"Localisation : {}".format(administratif["affectation"]))

        c.drawString(13*cm,25.2*cm,"Marque : {}".format(administratif["constructeur_enceinte"]))
        
        Application = administratif["application"]
        c.drawString(13*cm,24.2*cm,"Application : {}".format(Application))
        
        emt_processus = administratif["emt_processus"]
        c.drawString(13*cm,23.2*cm,"EMT processus : +/- {}".format(emt_processus))
        ...
        tableau_central = [[x[0], x[1], x[3], x[5], x[4]] for x in administratif["tableau_centrale"]]
...
#        gen_couleur =(couleur for couleur in self.couleur_dark.values())
        ...
        ##Rapport basé sur 15 sondes si moins il faut decaler:
        ...
        coordonnees_moyennes_cube= {"CH":(6.5, (9.3+decalage_cm)), "CP":(12.2, (9.3+decalage_cm)), 
                                                      ...
                                                      "CG":(3.5, 5.5+decalage_cm), "CD":(14.5, 6+decalage_cm), 
        ...
        coordonnees_u_cube= {"CH":(8.2, 9.3+decalage_cm), "CP":(14.2, 9.3+decalage_cm), 
                                                      ...
                                                      "CG":(4.8, 5.5+decalage_cm), "CD":(16.1, 6+decalage_cm), 
...
#        c.drawString(19*cm,1*cm,"Page 1/1")
        ...
        list_data = [ tuple(zip_longest(donnees["annexe"]["DONNEES"]["Date"].apply(self.format_date_secondes), donnees["annexe"]["DONNEES"][clef].tolist()))  for clef in donnees["annexe"]["DONNEES"] if clef !="Date" ]
        ...
        date_fin = donnees["annexe"]["DONNEES"]["Date"][index_fin - 1].strftime("%d-%m-%Y %H:%M:%S")
        nbr_mesure = len(donnees["annexe"]["DONNEES"][index_debut:index_fin]["Date"])

        
        list_data_selec = [ tuple(zip_longest(donnees["annexe"]["DONNEES"][index_debut:index_fin]["Date"].apply(self.format_date_secondes), donnees["annexe"]["DONNEES"][index_debut:index_fin][clef].tolist()))  for clef in donnees["annexe"]["DONNEES"] if clef !="Date" ]
...
#        c.rect(7*cm, 27*cm, 7*cm, 1.5*cm , stroke=1, fill=0)
        ...
        objet_remarques = "Evolution de la témperature simulée d'un CGR en fonction de la témperature de l'air de chaque capteur \n dans l'enceinte (en régime établi):"
        text_ob.textLines(objet_remarques)
        c.drawText(text_ob)
