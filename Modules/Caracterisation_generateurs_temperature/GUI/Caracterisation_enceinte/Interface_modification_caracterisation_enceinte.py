...
"""
Module implementing Modification_caracterisation_enceinte.
"""
...
from .Ui_Interface_modification_caracterisation_enceinte import Ui_Modification_caracterisation_enceinte
...
class Modification_caracterisation_enceinte(QMainWindow, Ui_Modification_caracterisation_enceinte):
    """
    Class documentation goes here.
    """
    def __init__(self, engine, meta, donnees_caracterisation, id_caract_select,   parent=None):
        """
        Constructor
        
        @param parent reference to the parent widget (QWidget)
        """
        super(Modification_caracterisation_enceinte, self).__init__(parent)
        self.setupUi(self)
        ...
        self.id_caracterisation = id_caract_select
...
#        print("index ^poly {}".format(index_poly))
        if index_poly != -1:
...
#        QtGui.QTableWidgetItem(str(nom_generateur))
        ...
        if self.lineEdit_temperature.text() !="":
            ...
            if num_pt - 1 < 1:
                ...
                for ligne in range(9):
                    #emplacement hom :
                    if float(self.tableWidget_mesures.item(ligne,5).text()) == float(self.lineEdit_hom_max_2.text()):
                ...
                if num_pt -1 != self.spinBox.value() and self.spinBox.value()>1:
                    self.actionSauvegarder.setEnabled(False)
...
#        print(self.lineEdit_temperature .text())
        if self.lineEdit_temperature.text() !="":
            ...
            if num_pt + 1 > self.spinBox.value():
                ...
                self.sauvegarde_maximum_par_pt[ self.label_pt.text()]["POSIT_HOMOGENEITE"] = None
                for ligne in range(9):
#                    print(" valeur tableau {} valeur à trouver {}".format(float(self.tableWidget_mesures.item(ligne,5).text()), float(self.lineEdit_hom_max_2.text())))
                    #emplacement hom :
                    if float(self.tableWidget_mesures.item(ligne,5).text()) == float(self.lineEdit_hom_max_2.text()) :
                        self.sauvegarde_maximum_par_pt[ self.label_pt.text()]["POSIT_HOMOGENEITE"] = self.tableWidget_mesures.verticalHeaderItem(ligne).text()
                    

#                print(self.sauvegarde_maximum_par_pt[ self.label_pt.text()]["POSIT_HOMOGENEITE"])
                #effacement
                for ligne in range(11):
                    for colonne in range(8):
                        if colonne !=6:    
...
#        print("self.poly dans le combobox  {}".format(self.poly))
...
#                print("self.idpoly {}".format(self.id_poly))
...
#        self.tableWidget_u_etalon.setItem(1, 1, item_2)  
        ...
        item_5.setFlags( QtCore.Qt.ItemIsSelectable |  QtCore.Qt.ItemIsEnabled )
...
#            
    ...
    def on_spinBox_valueChanged(self, p0):
        ...
        """
        self.nbr_pts_caracterisation = self.spinBox.value()
#        print("self.label_pt.text {}".format(self.label_pt.text()))
...
#        print(u_moyens)
    ...
    def on_actionSauvegarder_triggered(self):
        """
        permet la mise a jour des donnees
        """
        if self.lineEdit_temperature.text():
            ...
            self.db_carac.caracterisation_generateurs_admin_update(self.id_caracterisation, self.sauvegarde["admin"])
            
            
            self.sauvegarde["moyens_mesure"]["ID_CARACTERISATION"] = self.id_caracterisation               
    
            self.db_carac.caracterisation_generateurs_moyens_mesure_update(self.id_caracterisation,self.sauvegarde["moyens_mesure"])
            
            self.db_carac.caracterisation_enceinte_mesure_sup(self.id_caracterisation) #on efface toutes les donnees dans la table et on recree
            
            for i in range(1, self.spinBox.value()+1):
                for ele in self.sauvegarde[str(i)]:
                    ele["ID_CARACTERISATION"] = self.id_caracterisation
...
#        
            ...
            caracterisation_resultat = {"ID_CARACT" : self.id_caracterisation , "STABILITE" : max_stab_max, "TEMP_STAB" : temp_max_stab, 
...
#                        
            self.db_carac.caracterisation_enceinte_resultats_update(self.id_caracterisation,caracterisation_resultat)
            
        #gestion rapport
            if dossier :
                nom_fichier = "caracterisation n"+ " " + str(self.id_caracterisation) + " " + str(self.sauvegarde["admin"]["DATE"])
...
#                
            ...
            self.emit(SIGNAL("fermeturemodif_caracterisation_enceinte(PyQt_PyObject)"),self )
            self.close()
